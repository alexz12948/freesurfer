 ...
 *
 * Description:
 * This program is designed to take in a surface, two overlay files,
 * one or multiple volume files, one or multiple streamline files, and
 * an output directory. Based on the streamlines, output metrics will be
 * placed into a CSV file with the name of the original file and include
 * metrics such as curvature, thickness, and FA values.
 *
 */
...
// HELPER FUNCTIONS
float calc_mean(vector<float> n);
float calc_stde(vector<float> n, float mean);
string makeCSV(string dir, string file);
vtkIdType which_ID(double dist1, double dist2, vtkIdType ID1, vtkIdType ID2);
...
int main(int narg, char* arg[]) {
	...
	if ((num1.size() <= 8) or (num1.search(2, "--help", "-h"))) {
		cerr << "Usage: " << endl << arg[0] << " -i streamlineFile.trk"
				 << " -sl surfaceFile_lh.orig -tl overlayFile_lh.thickness"
				 << " -cl overlayFile_lh.curv" << endl
		     << " -sr surfaceFile_rh.orig -tr overlayFile_rh.thickness"
				 << " -cr overlayFile_rh.curv" << endl
				 << " -o outputDirectory" << endl << "-ri reference_image"
				 << "(NOTE: only use reference image when FA is not used" << endl
	...
	for(int i = 0; i < meshes->size(); i++) {
		...
		oFile << "Streamline Name , Curvature of Start Point , "
					<< "Curvature of Last Point , Thickness of Start Point ,"
					<< "Thickness of Last Point";
		if (FA_FOUND) {
			for (int a = 0; a < image_fileNames.size(); a++) {
				oFile << ", mean" << image_fileNames.at(a)
							<< ", stde" << image_fileNames.at(a);
			}
		}
		oFile << endl;
		...
		for (; inputCellIt != input->GetCells()->End(); ++inputCellIt, ++counter) {
			...
			if (FA_FOUND) {
				for (int p = 0; p < volumes.size(); p++) {
					...
					meanFA.push_back(calc_mean(FA_values));
					stdeFA.push_back(calc_stde(FA_values, meanFA.at(p)));
				}
      ...
      ref_Image.at(0)->TransformPhysicalPointToIndex(firstPt, first_index);
    	...
    	MRIvoxelToSurfaceRAS(image, first_index[0], first_index[1], first_index[2],
													 &firstPt_array[0], &firstPt_array[1], &firstPt_array[2]);
    	MRIvoxelToSurfaceRAS(image, last_index[0], last_index[1], last_index[2],
													 &lastPt_array[0], &lastPt_array[1], &lastPt_array[2]);
 ...
 * Does: Returns the vertice of the shorter distance
 */
vtkIdType which_ID(double dist1, double dist2, vtkIdType ID1, vtkIdType ID2)
{
...
vtkSmartPointer<vtkPolyData> FSToVTK(MRIS* surf)
{
	...
	for(int i = 0; i < surf->nvertices; i++) {
		points->InsertNextPoint(surf->vertices[i].x,
														surf->vertices[i].y,
														surf->vertices[i].z);
	}
