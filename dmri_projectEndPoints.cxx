 ...
 *
 * Description:
 * Changes the endpoints of a streamline to a different value
 * and save it to a new overlay file
 *
 */
...
int main(int narg, char* arg[]) {
	...
	if ((gp.size() <= 8) or (gp.search(2, "--help", "-h"))) {
  	cerr << "Usage: " << endl << arg[0] << " -i streamlineFile.trk"
				 << "-sl surfaceFile_lh.orig -sr surfaceFile_rh.orig" << endl
  ...
  ColorMeshType::CellsContainer::Iterator  inputCellIt = input->GetCells()->Begin();
    ...
    input->GetPoint(*it, &point);
		...
		MRIvoxelToSurfaceRAS(image, index[0], index[1], index[2],
    ...
    vtkIdType ID = which_ID(distL, distR, Left_ID, Right_ID);
		...
		if (ID == Left_ID) surfL->vertices[ID].curv = ENDPOINT_VALUE;
		else							 surfR->vertices[ID].curv = ENDPOINT_VALUE;
